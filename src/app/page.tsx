...
import { INITIAL_STATE, HONORIFICS, REGIONS_AND_CITIES, AA_SUBCITIES, EVIDENCE_LOCATIONS, DOCUMENT_ISSUERS, TEMPLATE_DATA, EVIDENCE_REGISTRY, TEMPLATES } from '@/lib/data';

// Mock AI flows for Static Export (Spark Plan) - Logic moved entirely to client
async function suggestEvidence(input: any): Promise<{ suggestedEvidence: string[] }> {
  console.log("AI Suggestion skipped in Static Mode");
  return { suggestedEvidence: [] };
}

async function provideMaintenanceContext(input: any): Promise<{ context: string }> {
  console.log("AI Maintenance Context skipped in Static Mode");
  return { context: "AI context unavailable in offline mode." };
}
